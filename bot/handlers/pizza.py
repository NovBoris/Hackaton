...
async def pizza_order_name(message: types.Message,state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        try:
            for i in menu_data:
                if i[2] == 'pizza' and i[0] == data['name']:
            ...
            pizzas.addNewPizza([some_data['id'],some_data['photo'],some_data['username'],some_data['userpass'],some_data['user_adress'],some_data['name'],some_data['price'],some_data['ingridients']],'delivery.db')
