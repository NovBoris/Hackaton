...
async def admin_userpass(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if admin.CheckUserInfo(data['username'], data['userpass'], 'delivery.db') == [
...
async def add_good_ingridients(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        menu.addNewProduct([data['name'], data['photo'], data['type'], data['price'], data['ingridients']],
                           'delivery.db')
    await state.finish()
...
Имя пользователя: {item[2]},
Адресс доставки: {item[4]},
Наименование товара: {item[5]},
Цена товара: {item[6]},
Ингридиенты: {item[7]}'''
        await bot.send_photo(message.from_user.id, item[1], caption=answer1, reply_markup=inline_delete_roll_kb_admin)
...
Цена товара: {item[6]},
Ингридиенты: {item[7]}'''
        await bot.send_photo(message.from_user.id, item[1], caption=answer2, reply_markup=inline_delete_pizza_kb_admin)
...
Цена товара: {item[6]},
Ингридиенты: {item[7]}'''
        await bot.send_photo(message.from_user.id, item[1], caption=answer3, reply_markup=inline_delete_burger_kb_admin)


async def delete_order_burger_start(callback: types.CallbackQuery):
    await oth.delete_admin_order_burger.id.set()
    await callback.message.reply('Введите id ДЗ: ')
...
async def delete_order_burger(message: types.Message, state: FSMContext):
    try:
        ...
        await message.reply('ДЗ с таким id не существует')
    finally:
        await state.finish()
...
def register_admin_message_handlers(dp: Dispatcher):
    ...
    dp.register_message_handler(add_good_photo, content_types=['photo'], state=oth.add_good.photo)
